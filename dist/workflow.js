...
function formatParticipationTime(dateIso) {
    if (!dateIso)
        return '';
...
export function normalizeWorkflowTemplateBody(body) {
    return body
        .replace(/\\r\\n/g, '\n')
        .replace(/\\n/g, '\n')
        .replace(/:relaxed:/g, '');
}
export function renderWorkflowTemplate(body, values) {
...
async function setApplicationStatus(application, status, patch = {}) {
...
async function insertDeliveryAttempt(input) {
    ...
    if (!error)
        return;
    if (/delivery_attempts|relation .* does not exist/i.test(error.message ?? ''))
        return;
...
async function recordOutgoing(application, text, action) {
...
async function getRegistrationState(studentId) {
    ...
    if (!error)
        return data;
    if (/student_registration_states|relation .* does not exist/i.test(error.message ?? ''))
        return null;
    throw error;
}
...
async function candidateApplications(studentId, intent) {
    let query = supabase
        .from('referral_applications')
    ...
    if (intent !== 'form_answered')
        return applications;
    const participationFormCandidates = applications.filter((application) => (application.post_participation_form_sent_at && !application.post_participation_form_answered_at));
    if (participationFormCandidates.length > 0)
        return participationFormCandidates;
    return applications.filter((application) => ((application.bank_form_sent_at || application.students?.bank_form_sent_at)
        && !application.bank_form_answered_at
        && !application.students?.bank_form_answered_at));
}
export async function rebuildWorkflowJobs(input = {}) {
    let query = supabase
        .from('referral_applications')
        .select('*')
        .eq('client_id', config.DEFAULT_CLIENT_ID)
...
function statusPatchForJob(job) {
    const sentAt = nowIso();
    if (job.job_type === 'pre_participation_caution') {
        return { status: 'pre_caution_confirmation_waiting', patch: { last_line_sent_at: sentAt } };
    }
    if (job.job_type === 'same_day_participation_reminder') {
        return { status: 'same_day_reminder_sent', patch: { same_day_reminder_sent_at: sentAt, last_line_sent_at: sentAt } };
    }
    if (job.job_type === 'post_participation_form') {
        return { status: 'post_participation_form_waiting', patch: { post_participation_form_sent_at: sentAt, last_line_sent_at: sentAt } };
    }
    if (job.job_type === 'bank_account_form') {
        return { status: 'bank_account_waiting', patch: { bank_form_sent_at: sentAt, last_line_sent_at: sentAt } };
    }
    return { status: null, patch: { last_line_sent_at: sentAt } };
}
function applicationCustomerDetails(application) {
...
function formatWorkflowSchedule(application) {
...
function workflowJobTypeLabel(jobType) {
    if (jobType === 'pre_participation_caution')
        return '参加前注意事項';
    if (jobType === 'same_day_participation_reminder')
        return '当日リマインド';
    if (jobType === 'post_participation_form')
        return '参加後フォーム案内';
    if (jobType === 'bank_account_form')
        return 'TS/銀行口座フォーム案内';
    return jobType;
}
function isDryRunApplication(application) {
    const values = [
...
function workflowApprovalFallbackText(application) {
    const customer = applicationCustomerDetails(application);
    return [
        'LINE送信承認待ち',
...
function workflowSentSummary(application, template, text, dryRun) {
    const customer = applicationCustomerDetails(application);
...
export function workflowApprovalResultBlocks(result, edited = false) {
...
async function rejectDryRunApplicationJob(job, application) {
    await markJob(job.id, {
        status: 'skipped',
        error_message: 'Skipped test/dry-run application',
    });
}
function dryRunApplicationError(application) {
...
export async function runWorkflowTick(input = {}) {
    ...
    for (const job of jobs) {
        ...
        if (!application?.line_user_id) {
            if (!dryRun)
                await markJob(job.id, { status: 'failed', error_message: 'Missing application or LINE user id' });
...
async function maybeSendBankForm(application, dryRun) {
    const student = application.students;
    const registration = await getRegistrationState(application.student_id);
    if (registration?.bank_form_answered_at || student?.bank_form_answered_at)
...
export async function processWorkflowReplyForApplication(input) {
    const dryRun = input.dryRun ?? config.LINE_SEND_DRY_RUN;
    ...
    if (input.intent === 'form_answered') {
        ...
        if (bank && 'text' in bank)
            sentTexts.push(bank.text);
        if (bank && 'alreadyAnswered' in bank && !dryRun)
            await setApplicationStatus(application, 'payment_ready');
        if (bank && 'alreadySent' in bank && !dryRun)
            await setApplicationStatus(application, 'bank_account_waiting');
    }
    if (!dryRun)
        await writeApplicationsToSheets({ applicationIds: [application.id], dryRun: config.SHEETS_WRITE_DRY_RUN });
    return { ok: true, dryRun, application, sentTexts };
}
export async function processWorkflowReply(input) {
    ...
    if (applications.length === 0) {
        return { handled: false, classification, reason: 'No matching referral application' };
    }
    if (applications.length > 1) {
        return { handled: true, needsSelection: true, classification, applications };
    }
    ...
    return { handled: true, needsSelection: false, classification, ...result };
}
export async function selectWorkflowApplication(input) {
    const { data, error } = await supabase
        .from('referral_applications')
        .select('*, students(id,line_user_id,display_name,bank_form_sent_at,bank_form_answered_at)')
        .eq('client_id', config.DEFAULT_CLIENT_ID)
...
export function planWorkflowJobsForSmoke(participationScheduledAt, applicationId = 'app-smoke') {
    return [
        {
            applicationId,
