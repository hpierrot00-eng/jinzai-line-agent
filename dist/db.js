...
function approvedKnowledgeSource(approvalId) {
    return `approved_reply:${approvalId}`;
}
export function buildKnowledgeBody(input) {
...
function normalizeIncomingRawPayload(input) {
    if (!input.markAsReadToken)
        return input.rawPayload ?? {};
    const raw = input.rawPayload;
    if (raw && typeof raw === 'object' && !Array.isArray(raw)) {
        return { ...raw, markAsReadToken: input.markAsReadToken };
    }
    return { rawPayload: raw ?? null, markAsReadToken: input.markAsReadToken };
}
export async function getMessageMarkAsReadToken(messageId) {
    if (!messageId)
        return null;
    const { data, error } = await supabase
        .from('messages')
...
export async function getReplyDraftMarkAsReadToken(replyDraftId) {
    const { data, error } = await supabase
        .from('reply_drafts')
    ...
    if (error) {
        ...
        if (/version|send_mode|updated_by|approved_by|approved_at|schema cache/i.test(error.message ?? '')) {
            let legacyQuery = supabase
                ...
                .limit(limit);
            if (category)
                legacyQuery = legacyQuery.in('category', [category, 'general']);
...
export async function approveMessageTemplate(key, input) {
    const { data, error } = await supabase
        .from('message_templates')
        .update({
        status: input.status ?? 'active',
...
async function getIncomingTextForDraft(replyDraft) {
...
export async function createKnowledgeFromApprovedReply(input) {
    if (!input.finalText?.trim())
        return { ok: false, status: 'empty_reply' };
    ...
    if (existing.error)
        throw existing.error;
    if (existing.data)
        return { ok: true, status: 'exists', source, itemId: existing.data.id };
    ...
    if (input.dryRun)
        return { ok: true, dryRun: true, status: 'would_create', source, title, category, body };
    ...
    return { ok: true, status: 'created', source, item };
}
export async function promoteApprovedRepliesToKnowledge(input = {}) {
    ...
    for (const approval of data ?? []) {
        results.push(await createKnowledgeFromApprovedReply({
            ...
            replyDraft: approval.reply_drafts,
            finalText: approval.after_text,
            action: approval.action,
            dryRun: input.dryRun,
        }));
    }
    return { ok: true, dryRun: Boolean(input.dryRun), scanned: data?.length ?? 0, created: results.filter((item) => item.status === 'created' || item.status === 'would_create').length, results };
}
export async function importPastTalkExamples(input) {
    ...
    for (const example of examples) {
        ...
        if (!replyText.trim()) {
            results.push({ ok: false, status: 'empty_reply' });
            continue;
        }
        ...
        if (existing.error)
            throw existing.error;
        if (existing.data) {
            results.push({ ok: true, status: 'exists', source, itemId: existing.data.id });
            continue;
        }
        const payload = {
        ...
        if (input.dryRun) {
            results.push({ ok: true, dryRun: true, status: 'would_create', ...payload });
            continue;
        }
        const item = await createKnowledgeItem(payload);
        results.push({ ok: true, status: 'created', source, item });
    }
    return { ok: true, dryRun: Boolean(input.dryRun), received: examples.length, created: results.filter((item) => item.status === 'created' || item.status === 'would_create').length, results };
}
