...
function extractSchedule(text) {
    ...
    if (dateLike)
        data.date_text = dateLike[0];
    if (timeLike)
        data.time_text = timeLike[0];
    if (/2回目|二回目|次回/.test(text))
        data.appointment_type = 'second_meeting';
    if (/初回|エージェント|面談/.test(text))
        data.appointment_type = data.appointment_type ?? 'first_agent_meeting';
    const school = text.match(/([^、。\s]+大学|[^、。\s]+短大|[^、。\s]+専門学校)/);
    if (school)
        data.school_name = school[1];
    const grad = text.match(/(20\d{2})年卒/);
    if (grad)
        data.graduation_year = grad[1];
    return data;
}
function fallbackDraft(input) {
    ...
    if (!monthlyAnswer && !templateReply && topKnowledge && base.risk_level !== 'blocked' && base.risk_level !== 'high')
...
function selectTemplate(templates = [], category) {
...
function monthlyRuleAnswer(rules) {
    if (rules.length === 0)
        return null;
...
function fallbackRevision(currentDraftText, instruction, monthlyRules = []) {
...
export async function generateDraft(input) {
    ...
    if (!res.ok)
        throw new Error(`OpenClaw draft failed: ${res.status} ${await res.text()}`);
    const json = await res.json();
    if (fallback.risk_level === 'blocked' || fallback.risk_level === 'high')
        return fallback;
    return {
...
export async function generateRevisionDraft(input) {
    ...
    if (!res.ok)
        throw new Error(`OpenClaw revision failed: ${res.status} ${await res.text()}`);
    const json = await res.json();
    if (fallback.risk_level === 'blocked' || fallback.risk_level === 'high')
        return fallback;
    return {
