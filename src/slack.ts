...
import { approveWorkflowJob, getWorkflowApprovalDraft, selectWorkflowApplication, workflowApprovalResultBlocks, type WorkflowIntent } from './workflow.js';
...
function formResponseLine(result: any, kind: 'post' | 'bank') {
  ...
  const candidates = (result.candidates ?? []).slice(0, 5).map((candidate: any) => {
    if (kind === 'post') return `${candidate.application_id ?? candidate.id} / ${candidate.student_name ?? candidate.students?.name ?? '名前なし'} / ${candidate.agent_name ?? '案件なし'}`;
...
export async function postFormResponseMatchNotification(syncResult: any) {
  ...
  if (!result.ok) throw new Error(`Slack form response notification failed: ${result.error}`);
  return result;
}
  ...
  if (actionId === 'workflow_approve_send') {
    ...
    try {
      ...
      await slack.chat.update({
        ...
        blocks: workflowApprovalResultBlocks(result) as any,
      });
    } catch (err) {
      await slack.chat.update({
        channel: payload.channel.id,
  ...
  if (payload.view.callback_id === 'workflow_edit_send_modal') {
    ...
    try {
      ...
      await slack.chat.update({
        ...
        blocks: workflowApprovalResultBlocks(result, true) as any,
      });
    } catch (err) {
      await slack.chat.update({
        channel: meta.channel,
