...
async function studentIdsWithActivePreCautionJobs(studentIds: string[]) {
  ...
  if (error) throw error;
  return new Set((data ?? []).map((row: { student_id: string | null }) => row.student_id).filter(Boolean) as string[]);
}

async function shouldSkipDuplicatePreCautionJob(job: WorkflowJob) {
  if (job.job_type !== 'pre_participation_caution') return false;
  ...
  if ((coveredApplications ?? []).length > 0) return true;
  ...
  const otherActiveJob = (activeJobs ?? []).find((activeJob: { id: string; status: string | null }) => (
    activeJob.id !== job.id && activeJob.status !== 'scheduled'
  ));
  if (otherActiveJob) return true;
  if (job.status && job.status !== 'scheduled') return false;
  const firstScheduledJob = (activeJobs ?? []).find((activeJob: { status: string | null }) => activeJob.status === 'scheduled');
...
export async function runWorkflowTick(input: { limit?: number; dryRun?: boolean } = {}) {
  ...
  for (const job of jobs) {
    ...
    if (await shouldSkipDuplicatePreCautionJob(job)) {
      if (!dryRun) await markJob(job.id, { status: 'skipped', error_message: 'Pre-participation caution already covered for this student' });
