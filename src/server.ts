...
function requireAdmin(req: express.Request, res: express.Response) {
  if (!config.ADMIN_API_KEY) return true;
  if (req.header('authorization') === `Bearer ${config.ADMIN_API_KEY}`) return true;
...
app.post('/webhooks/line', async (req, res, next) => {
  try {
    ...
    if (!verifyLineSignature(raw, req.header('x-line-signature') ?? undefined)) return res.status(401).send('invalid line signature');
